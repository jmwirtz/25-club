...
type RankMode = "totalGain" | "annualized";
...
export function MembersTable({ stats }: { stats: MemberStats[] }) {
  const [rankMode, setRankMode] = useState<RankMode>("totalGain");
  ...
  const rankMap = useMemo(() => {
    const ranked = [...stats].sort((a, b) =>
      rankMode === "annualized"
        ? b.weightedAvgAnnualizedReturnPct - a.weightedAvgAnnualizedReturnPct
  ...
  function switchRankMode(mode: RankMode) {
    setRankMode(mode);
    setSortKey(mode === "annualized" ? "weightedAvgAnnualizedReturnPct" : "totalGain");
    setSortDir("desc");
  }
  ...
  return (
    <div>
      <div className="px-4 py-2 border-b border-slate-100 flex items-center gap-2 text-sm">
        <span className="text-slate-500 text-xs">Rank by:</span>
        <button
          onClick={() => switchRankMode("totalGain")}
        ...
        <button
          onClick={() => switchRankMode("annualized")}
          className={`px-3 py-1 rounded text-xs font-medium transition ${
            rankMode === "annualized"
              ? "bg-blue-600 text-white"
              : "bg-slate-100 text-slate-600 hover:bg-slate-200"
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-slate-600 select-none">
            <tr>
              ...
              <Th sortKey="totalGainPct" current={sortKey} dir={sortDir} onClick={toggleSort} right>Total %</Th>
              <Th sortKey="weightedAvgAnnualizedReturnPct" current={sortKey} dir={sortDir} onClick={toggleSort} right>Ann. %</Th>
              <Th sortKey="shareOfClubGain" current={sortKey} dir={sortDir} onClick={toggleSort} right>% of Club</Th>
            ...
            {sorted.map((s) => {
              ...
              return (
                <tr key={s.member} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className={`px-3 py-2 text-right font-medium ${s.weightedAvgAnnualizedReturnPct >= 0 ? "text-emerald-700" : "text-rose-700"}`}>
                    {s.weightedAvgAnnualizedReturnPct >= 0 ? "+" : ""}
                    {s.weightedAvgAnnualizedReturnPct.toFixed(1)}%
                  </td>
